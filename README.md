...
location (“C:/Program Files (x86)/Joinpoint Command/jpCommand.exe”), you
...
This package includes a part of the sample dataset provided with the
software:
...
You can use `run_options()` and `export_options()` to setup parameters
...
Note that you can leave the options as default, and that the standard
error can be left unfilled:
...
You can use `jp_plot()` to plot the joinpoint lines along the scatter
points:
